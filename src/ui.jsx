...
const ThemeToggle = () => {
  ...
  return (
    <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title="Toggle theme"
      style={{width:36, height:36, borderRadius:"50%", border:"1px solid var(--line-2)",
...
const Footer = () => (
  <footer style={{marginTop:120, borderTop:"1px solid var(--line)", padding:"56px 0 40px"}}>
    <div className="wrap" style={{display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr", gap:48}}>
      <div>
        ...
        <p style={{marginTop:20, maxWidth:340, color:"var(--ink-2)", fontSize:14, lineHeight:1.55}}>
          Bepul, ochiq bilim platformasi — xalq tomonidan yaratilgan, Google tomonidan indekslangan.
        </p>
      ...
      {[
        ["Haqida", ["Bizning missiya","Tahririyat standartlari","Jamoa va matbuot","Aloqa"]],
        ["Qanday ishlaydi", ["Profil yaratish","Tasdiqlash darajalari","SEO indekslash","API kirish"]],
        ["Qo'llanmalar", ["SEO qo'llanma","Strukturlangan ma'lumot","Manbalar","Uslub qo'llanmasi"]],
        ["Huquqiy", ["Maxfiylik","Shartlar","O'chirish","Cookie"]],
      ].map(([title, items]) => (
        <div key={title}>
    ...
    <div className="wrap" style={{marginTop:56, paddingTop:24, borderTop:"1px solid var(--line)",
      display:"flex", justifyContent:"space-between", alignItems:"center", color:"var(--ink-3)", fontSize:12}}>
      <span>© 2026 WikiBio Foundation. CC BY-SA 4.0 litsenziyasi.</span>
...
const ProfileCard = ({ profile, variant = "default", index = 0 }) => {
  ...
  // DB field nomlarini normalize qilish
  const name = profile.name || "";
  const photo = profile.photo_url || profile.photo || null;
  const shortBio = profile.short_bio || profile.short || "";
  const birthYear = profile.birth_date ? profile.birth_date.toString().slice(0,4) : (profile.born || "");
  const nationality = profile.nationality || "";
  const views = profile.views_count || profile.views || 0;
  const slug = profile.slug || profile.id;

  // Oxirgi so'zni italic qilish
  ...
  return (
    <a href={`#/p/${slug}`}
      ...
      <div className="photo-frame" style={{height:h, position:"relative"}}>
        <Photo src={photo} name={name} color="var(--gold-3)"
        ...
        <div style={{position:"absolute", bottom:18, left:18, right:18, zIndex:2}}>
          <div style={{
            ...
            fontWeight:300, lineHeight:0.98, color:"#fff", letterSpacing:"-0.02em"
          }}>
            {firstName} <span style={{fontStyle:"italic"}}>{lastName}</span>
          </div>
          {(birthYear || nationality) && (
            <div className="mono" style={{marginTop:8, fontSize:10, letterSpacing:".18em",
              textTransform:"uppercase", color:"rgba(255,255,255,0.75)"}}>
              {birthYear ? `b. ${birthYear}` : ""}{birthYear && nationality ? " · " : ""}{nationality}
            </div>
          )}
        </div>
      </div>
      <div style={{padding:"18px 4px 4px"}}>
        ...
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center",
          ...
          <span style={{color:"var(--gold)", display:"inline-flex", alignItems:"center", gap:4}}>
            Profil o'qish <I.arrowR size={11}/>
          </span>
        </div>
      </div>
    </a>
    ...
    <div className="wrap">
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-end", gap:40, flexWrap:"wrap"}}>
        <div>
          ...
          <h1 className="display" style={{margin:0, fontSize: dense ? 56 : 88, maxWidth:900}}>
...
Object.assign(window, { Logo, ThemeToggle, useTheme, TopNav, Footer, Photo, VerifiedBadge, ProfileCard, PageHeader, Pill });
